...
const PLUGIN = process.argv[2] ?? resolve(import.meta.dirname, "..");
...
if (files.length > 0) {
  const relToPlugin = files.map((f) => join(relative(PLUGIN, FIXTURES), f)).join("\n");
  const ci = spawnSync("git", ["-C", PLUGIN, "check-ignore", "--stdin"], {
    input: relToPlugin,
    encoding: "utf8",
  });
  if (ci.error) {
