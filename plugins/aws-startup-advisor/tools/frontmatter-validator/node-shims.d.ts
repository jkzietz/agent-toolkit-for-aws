...
/** Minimal stand-in for Node's Buffer — only the byte comparison the tools use. */
interface FileBytes {
...
// Node 20.11+ exposes the module's own directory here. fixtures-check.ts,
// pricing-staleness.ts and sync-vendored-shared.ts use it to locate the plugin root
// from their own location rather than a repo-relative path, so they run correctly from
// any working directory and survive the plugin moving inside a repo.
