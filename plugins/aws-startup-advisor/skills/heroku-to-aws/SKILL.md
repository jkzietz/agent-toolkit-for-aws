---
name: heroku-to-aws
description: "Migrate workloads from Heroku to AWS. Triggers on: migrate from Heroku, Heroku to AWS, move off Heroku, migrate Heroku app, migrate Heroku Postgres to RDS, migrate Heroku Redis to ElastiCache, migrate Heroku Kafka to MSK, migrate dynos to Elastic Beanstalk, migrate dynos to Fargate, Heroku migration, migrate Heroku Private Space, Heroku to ECS, leave Heroku, what-if workshop, reprice Heroku migration, compare migration scenarios, workshop mode. Runs a 6-phase process: discover (live via the read-only, consent-gated Heroku CLI and/or Terraform, Procfile, app.json), clarify, design, estimate, generate, feedback — Clarify must finish before Design, Estimate, or Generate, and an optional what-if workshop reprices region/HA/compute/Graviton scenarios after Estimate. Do not use for: GCP or Azure migrations to AWS, AWS-to-Heroku reverse migration, general AWS architecture advice without migration intent, Heroku-to-Heroku refactoring, or multi-cloud deployments that do not involve migrating off Heroku."
---
