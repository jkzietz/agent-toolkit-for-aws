...
2. Fallback for anything missing: the AWS Price List API if available.
3. Record `pricing_source`: `cached` | `cached_stale` (if >30 days old) | `mcp`.

Cached anchors (order-of-magnitude, us-east-1, verify; last updated 2025-07-14 — refresh via the AWS Price List API when >30 days old):
...
- AgentCore (Instances compute type, `agentcore_compute_type: "instances"` in design.json):
  ...
  anchor here: pull the EC2 rate via the AWS Price List API (instance type comes from the user's
...
instance is unknown, fall through to the AWS Price List API rather than assuming Fargate. A W1
...
- **agent_session units** (effective_runtime = agentcore | lambda_microvms | ecs | eks | lambda):
  ...
  anchor falls through to the AWS Price List API.
...
- **temporal_worker_poll units** (verdict from Tier 1: ecs / eks / serverless_workers): estimate the
  ...
  - `effective_runtime == "serverless_workers"` → Temporal Serverless Workers is **Public
    ...
    without a GA announcement). Do NOT invent a cached anchor: try the AWS Price List API for its
...
polling tier whose rate the AWS Price List API could not confirm): set that component to the string
