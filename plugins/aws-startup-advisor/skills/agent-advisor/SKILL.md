---
name: agent-advisor
description: "Unified entry point for AI-agent work on AWS: pick a runtime, generate a migration plan for an existing workload, and build an executable POC — all in one flow. Triggers on: which runtime for my agent, AgentCore vs ECS vs EKS vs Lambda, AgentCore vs Lambda MicroVMs, deploy an AI agent on AWS, agent architecture on AWS, I have an agent idea what do I build, move or migrate my agents to AWS, agent migration plan, add AgentCore services, add memory/gateway/identity/policy to my agent, add observability to my agent, already on AWS and want to add agent capabilities, migrate Temporal workers to AWS, Temporal to AWS, run Temporal on AWS, Temporal Cloud or self-hosted on AWS. Requires at least one agentic component: a purely non-agent system (only plain services or batch jobs) is out of scope and is routed to gcp-to-aws, heroku-to-aws, or llm-to-bedrock. Not for: pure compute or data migration with no AI agent; a pure LLM SDK rewrite without agent architecture (use llm-to-bedrock); or detailed per-model pricing."
---
