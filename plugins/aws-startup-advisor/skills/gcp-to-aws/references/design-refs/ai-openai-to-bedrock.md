...
choice, not a provider comparison. Verify rates via `shared/pricing-cache.md` or the AWS Price List API, noting that
the Price List API does not carry GPT-5.x (see `shared/openai-on-bedrock.md`).
